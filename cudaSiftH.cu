...
size_t SiftTempMem::aprroximate_size(int width, int height, int numOctaves)
{
    const size_t nd = NUM_SCALES + 3;
    size_t w = width;
    size_t h = height;
    size_t p = iAlignUp(w, 128);
    size_t size = h * p;         // image sizes
    size_t sizeTmp = nd * h * p; // laplace buffer sizes
    for (int i = 0; i < numOctaves; i++)
    {
        w /= 2;
        h /= 2;
